...
const googleFontsApiKey = process.env.GOOGLE_FONTS_API_KEY || env.GOOGLE_FONTS_API_KEY || "";
const colorApiBaseUrl = process.env.COLOR_API_BASE_URL || env.COLOR_API_BASE_URL || "https://www.thecolorapi.com";
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && requestUrl.pathname === "/api/images/search") {
      ...
      sendJson(res, 200, { query, images: await searchUnsplash(query) });
      return;
    }

    if (req.method === "GET" && requestUrl.pathname === "/api/fonts") {
      const limit = clamp(Number(requestUrl.searchParams.get("limit") || 40), 1, 100);
      sendJson(res, 200, { fonts: await listGoogleFonts(limit) });
      return;
    }
    ...
    if (req.method === "GET" && requestUrl.pathname === "/api/colors/palette") {
      ...
      sendJson(res, 200, { hex, mode, palette: await getColorPalette(hex, mode, count) });
      return;
    }
...
async function searchUnsplash(query) {
  if (!hasRealKey(unsplashAccessKey)) return fallbackImages(query);
  ...
  if (!response.ok) return fallbackImages(query);
...
async function listGoogleFonts(limit) {
  if (!hasRealKey(googleFontsApiKey)) return fallbackFonts().slice(0, limit);
  ...
  const response = await fetch(url);
  if (!response.ok) return fallbackFonts().slice(0, limit);
...
async function getColorPalette(hex, mode, count) {
  ...
  const response = await fetch(url);
  if (!response.ok) return fallbackPalette(hex);
