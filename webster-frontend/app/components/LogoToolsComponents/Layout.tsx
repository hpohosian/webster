...
function LayoutPreview({ variant, active }: { variant: LayoutVariant; active: boolean }) {
  ...
  const iconBox = <rect width="18" height="18" rx="3" fill={iconColor} />;
  const textLines = (
    <g>
      <rect width="32" height="4" rx="2" fill={textColor} />
      <rect y="7" width="24" height="3" rx="1.5" fill={textColor} opacity="0.5" />
    </g>
  );
  ...
  switch (variant) {
    ...
    case "Icon-Top":
      content = (
        <svg viewBox="0 0 80 44" className="w-full">
