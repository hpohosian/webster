...
export type CanvasCommand =
  ...
  | { id: number; type: "add-image"; url: string; alt: string }
...
export type CanvasCommandInput = CanvasCommand extends infer T ? T extends { id: number } ? Omit<T, "id"> : never : never;
    ...
    (set, get) => ({
      ...
      setLayers: (layers, selectedLayerId) =>
        set(
          (s) => ({
            layers,
            selectedLayerId: selectedLayerId ?? (layers.some((l) => l.id === s.selectedLayerId) ? s.selectedLayerId : layers[layers.length - 1]?.id ?? ""),
          }),
          false,
          "setLayers"
        ),
